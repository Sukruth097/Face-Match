...
logging.basicConfig(filename=os.path.join(log_dir, 'RunningLogs.log'), level=logging.INFO, format=logging_str,
                    filemode="a")
...
def generate_data_pickle_file(config_path,params_path):
    ...
    logging.info(f"Total actor's images are: {len(filenames)}")
